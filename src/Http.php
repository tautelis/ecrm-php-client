...
class Http
{
     ...
     *
     * @return Response Array where first element is http status code and the second one is resource.
     */
     ...
     *  @option boolean 'raw' Indicator whether to wrap and uwrap request in the envelope.
     *
     * @throws \EnhancedCRM\Exception\ConnectionError if connnection error occurrs e.g. timeout, dns issue etc.
     * @throws \EnhancedCRM\Exception\RequestError if request was invalid
     * @throws \EnhancedCRM\Exception\ResourceError if request's payload validation failed
     * @throws \EnhancedCRM\Exception\ServerError if server error occurred
     *
     * @return Response Array where first element is http status code and the second one is resource.
     */
    public function request($method, $url, array $params = null, array $body = null, array $options = array())
    {
        ...
        if ($resp === false)
        {
            ...
            throw new Exception\ConnectionError($errno, $error_message);
        }
    ...
    protected function handleErrorResponse($code, $rawResponse, $response)
    {
        ...
        {
            case $code == 422:
                throw new Exception\ResourceError($code, $response);
            case $code == 429:
                throw new Exception\RateLimitError();
            case $code >= 400 && $code < 500:
                throw new Exception\RequestError($code, $response);
            case $code >= 500 && $code < 600:
                throw new Exception\ServerError($code, $response);
            default:
